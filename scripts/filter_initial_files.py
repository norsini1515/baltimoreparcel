...
def process_year(year: int, column_subset:Iterable[str]=PARCEL_FIELDS):
    try:
        ...
        gdf = gis.read_vector_layer(year, name=in_name, directory=RAW_DIR / f"{year}")
        if gdf is None:
            return
        ...
        #Ensure correct CRS
        gdf = gis.ensure_crs(gdf, epsg=26985)
        filtered_gdf = gis.filter_on_field(
            gdf,
        ...
        gis.write_gpkg_layer(filtered_gdf, year=year, name=gpkg_filename, directory=output_gpkg_dir, layer=f"{year}filtered")
        
        # Subset to relevant fields- easier to work with
        subset_gdf = gis.select_columns(filtered_gdf, columns=column_subset)
        gis.write_gpkg_layer(subset_gdf, year=year, name=gpkg_filename, directory=output_gpkg_dir, layer=f"{year}subset")
...
if __name__ == "__main__":
    ...
    for layer_name, cols in SUBSET_CONFIGS.items():
        gdf_subset = gis.select_columns(filtered_gdf , cols)
        gis.write_gpkg_layer(gdf_subset, year, gpkg_filename, output_gpkg_dir, layer=layer_name)
    '''